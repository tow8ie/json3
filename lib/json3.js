...
;(function (root, Module) {
  ...
  // Convenience aliases.
  var getClass = {}.toString, hasOwnProperty = {}.hasOwnProperty, stringifySupported, parseSupported, forEach, isArguments, propertyIsEnumerable;
    ...
    if (stringifySupported) {
      ...
      try {
        switch (false) {
          ...
          // The `arguments` object is inconsistently serialized across
          // environments, depending on whether its numeric properties are
          // enumerable.
          case exports.stringify(arguments) == '{"0":1}':
  ...
  // **isArguments** determines if the given object is an `arguments` object by
  // checking its `[[Class]]` name. Specified in ES 5 section 10.6; implemented
  // in Safari >= 2.0, Firefox >= 4.0, and Opera >= 11.61.
  isArguments = function isArguments(value) {
    return getClass.call(value) == "[object Arguments]";
  };
  if (!isArguments(arguments)) {
    // Safari <= 2.0.3 doesn't implement `Object#isPropertyEnumerable`,
    // but supports the `Arguments` `[[Class]]` name.
    propertyIsEnumerable = {}.propertyIsEnumerable;
    // Use duck typing to detect the `arguments` object in other
    // environments.
    isArguments = function isArguments(value) {
      return hasOwnProperty.call(value, "callee") && !(propertyIsEnumerable && propertyIsEnumerable.call(value, "callee"));
    };
  }
  ...
  forEach = (function () {
    var members, property, forEach, enumerableArguments = false, size = 0;

    // Test if the numeric properties of the `arguments` object are enumerable.
    // According to section 10.1.8 of the ES 3 spec, the properties should not
    // be enumerable; according to ES 5 section 10.6, they should. Results vary
    // widely across environments; this test is currently known to return `true`
    // only in Safari 2.x and Firefox >= 4.0.
    for (property in arguments) {
      if (hasOwnProperty.call(arguments, property) && property == "0") {
        enumerableArguments = true;
        break;
      }
    }
    ...
    if (!size) {
      ...
      // No bugs detected; use the standard `for...in` algorithm. If the
      // `inherited` argument is `true`, direct and inherited properties will
      // be enumerated.
      forEach = function forEach(object, callback, inherited) {
        var index, length, isFunction, property, isConstructor;
        if (!enumerableArguments && isArguments(object)) {
          // Manually enumerate over the `arguments` object if its numeric
          // properties are not enumerable in the current environment.
          ...
          for (property in object) {
            if (!(isFunction && property == "prototype") && (inherited || hasOwnProperty.call(object, property)) && !(isConstructor = property === "constructor")) {
              callback(property);
            }
          }
